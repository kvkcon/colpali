...
import torch
from datasets import Dataset, load_dataset, Features, Value, Sequence, Image, load_from_disk, concatenate_datasets
...
def main():
    ...
    dataset_list = []
    dataset_list.append(raw_dataset.filter(lambda x: x['source'] in config['dataset']['source']).map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True))
    dataset_list.append(raw_dataset.filter(lambda x: x['source'] not in config['dataset']['source']).map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True))
    # dataset_list.append(raw_dataset.filter(lambda x: x['source'] == 'docvqa').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True))
    # dataset_list.append(raw_dataset.filter(lambda x: x['source'] == 'arxiv_qa').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True))
    # dataset_list.append(raw_dataset.filter(lambda x: x['source'] == 'Infographic-VQA').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True))

    # dataset2 = raw_dataset.filter(lambda x: x['source'] == 'pdf').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True)
    # dataset3 = raw_dataset.filter(lambda x: x['source'] == 'docvqa').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True)
    # dataset4 = raw_dataset.filter(lambda x: x['source'] == 'arxiv_qa').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True)
    # dataset5 = raw_dataset.filter(lambda x: x['source'] == 'Infographic-VQA').map(lambda x, idx: {**x, 'original_index': idx}, with_indices=True)

    if data_source == 0:
        # handle dataset from pdf&tatdqa source
        # dataset = concatenate_datasets(dataset_list[:2])
    ...
    dataset = dataset_list[data_source]
            ...
            points.append(
                models.PointStruct(
                    ...
                    payload={
                        ...
                        "original_index": dataset[id]["original_index"],
    ...
    for batch_idx, batch_query in enumerate(tqdm(query_dataloader, desc="Calculating scores and selectin topK negative samples...")):
        ...
        for ki in range(len(embeddings_query)):
            ...
            positive_index_raw = dataset[positive_index]["original_index"]
            scores_k = [r.score for r in search_result.points]
            top_k_indices = [r.payload["original_index"] for r in search_result.points] 
            negative_indices = [idx for idx in top_k_indices if idx != positive_index_raw][:25]
            ...
            new_hf_dataset_dict["positive_index"].append(positive_index_raw)
